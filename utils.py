...
def get_keypoints():
    ...
    return keypoint_flip_map

def flip_keypoints(keypoint_flip_map, keypoint_coords, width):
    ...
    for lkp, rkp in keypoint_flip_map.items():
        flipped_kps[:, lkp, :] = keypoint_coords[:, rkp, :]
        flipped_kps[:, rkp, :] = keypoint_coords[:, lkp, :]

    # Flip x coordinates
    flipped_kps[:, :, 0] = width - flipped_kps[:, :, 0] - 1
    inds = np.where(flipped_kps[0, :, 2] == 0)
    flipped_kps[0, inds[0], 0] = 0
...
def unmold_keypoint_mask(keypoints_prob, image_shape, keypoint_mask_shape = (56,56), keypoint_threshold= 0.00):
    """Converts a mask generated by the neural network into a format similar
    to it's original shape.
